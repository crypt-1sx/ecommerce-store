...
async function storageSet(k,v){ try{ localStorage.setItem(k,JSON.stringify(v)) }catch(e){ console.error(e)} }
...
export default function App(){
  ...
  const load=useCallback(async()=>{
    ...
    if(!p){ p=SEED_PRODUCTS; await storageSet("dz-store-products",p) }
    else { p = p.map(x=>({quantity:20,...x})); }
    if(!o){ o=[]; await storageSet("dz-store-orders",o) }
    setProducts(p); setOrders(o); setLoading(false);
  },[]);
  useEffect(()=>{ load() },[load]);
  const saveProducts=async(n)=>{ setProducts(n); await storageSet("dz-store-products",n) };
  const saveOrders=async(n)=>{ setOrders(n); await storageSet("dz-store-orders",n) };
    ...
    <BrowserRouter>
      <div dir={lang==="ar"?"rtl":"ltr"} style={{background:"var(--paper-2)",minHeight:"100vh",padding:"10px"}}>
        <div style={{maxWidth:440,margin:"0 auto",background:"#fff",minHeight:"calc(100vh - 20px)",overflow:"hidden",border:"1px solid var(--line)",position:"relative"}}>
          <Routes>
            <Route path="/" element={<Storefront products={products} orders={orders} saveOrders={saveOrders} lang={lang} setLang={setLang} t={t} />} />
            <Route path="/admin/login" element={<AdminLogin />} />
            <Route path="/admin" element={<ProtectedRoute><Dashboard products={products} saveProducts={saveProducts} orders={orders} saveOrders={saveOrders} /></ProtectedRoute>} />
  ...
  return (
    <div className="fade-in" style={{padding:22,minHeight:"calc(100vh - 28px)",display:"flex",flexDirection:"column"}}>
      ...
      <form onSubmit={submit} style={{background:"#F8FAFC",border:"1px solid var(--line)",borderRadius:18,padding:16}}>
        ...
        <div className="mono" style={{marginTop:10,color:"#94A3B8",fontSize:11}}>الافتراضية: <b style={{color:"var(--ink)"}}>{ADMIN_PASSWORD}</b> — غيّرها من <code style={{background:"#fff",border:"1px solid var(--line)",padding:"2px 6px",borderRadius:6}}>src/App.jsx</code></div>
...
function Storefront({products,saveOrders,orders,lang,setLang,t}){
  const [selected,setSelected]=useState(null);
  const [ordering,setOrdering]=useState(false);
  const [confirmedId,setConfirmedId]=useState(null);
  const placeOrder=async(payload)=>{
    const order={id:"o"+Date.now(),...payload,status:"قيد الانتظار",createdAt:new Date().toISOString()};
    const next=[order,...orders]; await saveOrders(next); setConfirmedId(order.id); setOrdering(false); setSelected(null);
  };
  if(confirmedId) return <OrderConfirmed onClose={()=>setConfirmedId(null)} t={t} lang={lang} />;
  if(ordering && selected) return <OrderForm product={selected} onBack={()=>setOrdering(false)} onSubmit={placeOrder} t={t} lang={lang} />;
  if(selected) return <ProductDetail product={selected} onBack={()=>setSelected(null)} onOrder={()=>setOrdering(true)} t={t} lang={lang} />;
  return (
...
function ProductCard({p,onClick,t,lang}){
  return (
    <button onClick={onClick} style={{textAlign: lang==="ar"?"right":"left",background:"#fff",border:"1px solid var(--line)",overflow:"hidden",padding:0,cursor:"pointer",display:"flex",flexDirection:"column",transition:"border-color .15s ease"}}>
      <div style={{width:"100%",aspectRatio:"1/1",background:`#F3F3F0 url(${p.img}) center/cover`,borderBottom:"1px solid var(--line)"}}/>
      <div style={{padding:"10px 10px 11px",flex:1,display:"flex",flexDirection:"column",gap:6}}>
        ...
        <div style={{marginTop:"auto",display:"flex",alignItems:"center",justifyContent:"space-between",gap:8,borderTop:"1px solid var(--line-2)",paddingTop:8}}>
          ...
          <span style={{fontSize:10,fontWeight:700,letterSpacing:.6,color:varMuted,border:"1px solid var(--line)",padding:"3px 6px"}}>{t("view")}</span>
        </div>
      </div>
    </button>
  );
    ...
    <div className="fade-in" style={{paddingBottom:88}}>
      ...
      <div style={{padding:14}}>
        <div style={{background:"var(--sky-2)",border:"1px solid var(--line)",borderRadius:20,padding:10}}>
          <div style={{width:"100%",aspectRatio:"1/1",borderRadius:14,background:`#fff url(${product.img}) center/cover`,border:"1px solid var(--line)"}}/>
        </div>
        <div style={{marginTop:14}}>
          <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:10}}>
            <h1 style={{margin:0,fontSize:22,fontWeight:800,letterSpacing:-.5,lineHeight:1.25}}>{product.name}</h1>
            <span style={{background:"var(--red-soft)",border:"1px solid #FECACA",color:"var(--red)",fontSize:11,fontWeight:800,padding:"5px 9px",borderRadius:20,whiteSpace:"nowrap"}}>{t("available")}</span>
          </div>
      ...
      <div style={{position:"fixed",maxWidth:440,width:"100%",bottom:14,padding:"0 14px",boxSizing:"border-box"}}>
        <div style={{background:"var(--ink)",borderRadius:16,padding:10,display:"flex",gap:10,alignItems:"center",boxShadow:"0 16px 30px rgba(12,14,11,.22)",border:"1px solid #1e211e"}}>
          <div style={{flex:1,paddingInlineStart:6}}>
            <div style={{color:"#fff",fontWeight:800,fontSize:13,lineHeight:1}}>{t("orderNow")}</div>
            <div style={{color:"#94A3B8",fontSize:11,marginTop:2}}>{t("confirmPhone")}</div>
          </div>
          <button onClick={onOrder} style={{background:"var(--red)",color:"#fff",border:"none",borderRadius:12,padding:"13px 18px",fontWeight:800,fontSize:14,cursor:"pointer",display:"flex",alignItems:"center",gap:8,whiteSpace:"nowrap"}}>{t("continueOrder")} <ArrowRight size={14} style={{transform: lang==="ar"?"rotate(180deg)":"none"}}/></button>
        </div>
      </div>
    </div>
  );
...
function OrderForm({product,onBack,onSubmit,t,lang}){
  ...
  const submit=()=>{
    ...
    recordOrder();
    onSubmit({productId:product.id,productName:product.name,price:product.price,qty,total:product.price*qty,firstName:fn,lastName:ln,phone:phone.trim(),wilaya,delivery});
    ...
    <div className="fade-in" style={{paddingBottom:120}}>
      ...
      <div style={{padding:"0 14px"}}>
        <div style={{background:"#fff",border:"1px solid var(--line)",borderRadius:16,padding:12,display:"flex",gap:10,alignItems:"center",boxShadow:"var(--shadow-card)"}}>
          <div style={{width:54,height:54,borderRadius:12,background:`#fff url(${product.img}) center/cover`,border:"1px solid var(--line)",flexShrink:0}}/>
          <div style={{flex:1}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:8,background:"var(--ink)",borderRadius:12,padding:"6px 8px",color:"#fff"}}>
            <button onClick={()=>setQty(q=>Math.min(10,q+1))} style={{width:26,height:26,borderRadius:8,border:"1px solid #2a2e2a",background:"#1a1d1a",color:"#fff",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}}><Plus size={12}/></button>
...
function OrderConfirmed({onClose,t,lang}){
  const navigate=useNavigate();
  ...
  return (
    <div style={{minHeight:"calc(100vh - 28px)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:22,textAlign:"center"}} className="fade-in">
      ...
      <div style={{marginTop:14,background:"#F8FAFC",border:"1px solid var(--line)",borderRadius:14,padding:12,width:"100%",maxWidth:320,display:"flex",alignItems:"center",gap:10,textAlign: lang==="ar"?"right":"left"}}>
        ...
        <div style={{flex:1}}>
          <div style={{fontWeight:800,fontSize:12.5}}>{t("saved")}</div>
          <div className="mono" style={{fontSize:11,color:varMuted}}>COD</div>
        </div>
...
function Dashboard({products,saveProducts,orders,saveOrders}){
  const navigate=useNavigate();
  const [tab,setTab]=useState("orders");
  const [orderFilter,setOrderFilter]=useState("pending");
  const [showAdd,setShowAdd]=useState(false);
  const [editing,setEditing]=useState(null);
  const removeProduct=async(id)=>{ await saveProducts(products.filter(p=>p.id!==id)) };
  ...
  const setStatus=async(id,s)=>{ await saveOrders(orders.map(o=>o.id===id?{...o,status:s}:o)) };
  ...
  const revenue=orders.filter(o=>o.status!=="ملغى").reduce((a,b)=>a+b.total,0);
      ...
      <div style={{background:"var(--ink)",padding:"14px 14px 12px",position:"sticky",top:0,zIndex:5}}>
        ...
        <div style={{marginTop:12,display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
          ...
          <div style={{background:"var(--red)",borderRadius:14,padding:10}}>
            <div className="mono" style={{color:"rgba(255,255,255,.85)",fontSize:10,letterSpacing:.6}}>REVENUE</div><div style={{color:"#fff",fontWeight:800,fontSize:14,marginTop:2}}>{riyal(revenue)}</div><div style={{color:"rgba(255,255,255,.8)",fontSize:11}}>إجمالي</div>
          </div>
        </div>
      </div>
          ...
          <div style={{marginTop:10,display:"grid",gap:8}}>
            {products.map(p=>(
              <div key={p.id} style={{display:"flex",gap:10,alignItems:"center",background:"#fff",border:"1px solid var(--line)",borderRadius:14,padding:10,boxShadow:"var(--shadow-card)"}}>
                <div style={{width:56,height:56,borderRadius:11,background:`#fff url(${p.img}) center/cover`,border:"1px solid var(--line)",flexShrink:0}}/>
                ...
                <div style={{display:"flex",gap:6}}>
                  ...
                  <button onClick={()=>removeProduct(p.id)} style={{width:34,height:34,borderRadius:10,background:"#FFF1F2",border:"1px solid #FECACA",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}}><Trash2 size={14} color="var(--red)"/></button>
                </div>
              </div>
            ))}
          </div>
...
function AddProductModal({onClose,onAdd}){
  ...
  const handleFile=(e)=>{
    ...
    if(file.size> 2*1024*1024) return setErr("حجم الصورة كبير (max 2MB)");
    const reader=new FileReader();
    reader.onload=()=>{ setImg(reader.result); setPreview(reader.result); setErr("") };
    reader.readAsDataURL(file);
  };
  const submit=()=>{
    if(!name.trim()||!price) return setErr("أدخل الاسم والسعر");
    if(!img) return setErr("حمّل صورة المنتج");
    onAdd({name,price:Number(price),quantity:Number(quantity)||0,desc,img});
  };
  const s=fieldStyle();
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(12,14,11,.5)",backdropFilter:"blur(6px)",display:"flex",alignItems:"flex-end",zIndex:50}}>
      <div style={{maxWidth:440,width:"100%",margin:"0 auto",background:"#fff",borderRadius:"20px 20px 0 0",padding:16,border:"1px solid var(--line)",boxShadow:"0 -16px 40px rgba(0,0,0,.18)",maxHeight:"92vh",overflowY:"auto"}}>
        ...
        <label style={{display:"flex",alignItems:"center",gap:10,background:"#F8FAFC",border:"1px dashed var(--line)",borderRadius:12,padding:10,cursor:"pointer",marginBottom:10}}>
          <div style={{width:48,height:48,borderRadius:10,background:preview?`url(${preview}) center/cover`:"#fff",border:"1px solid var(--line)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>{!preview && <ImageIcon size={16} color={varMuted}/>}</div>
          <div style={{flex:1}}>
            <div style={{fontWeight:700,fontSize:12.5,display:"flex",alignItems:"center",gap:6}}><Upload size={13}/> {preview?"تم التحميل — اختر أخرى":"حمّل صورة (JPG/PNG)"}</div>
            <div className="mono" style={{fontSize:11,color:varMuted}}>max 2MB — تُحفظ في الجهاز</div>
          </div>
        ...
        {err && <div style={{marginTop:10,background:"var(--red-soft)",border:"1px solid #FECACA",color:"var(--red)",fontSize:12.5,fontWeight:700,borderRadius:10,padding:"9px 11px"}}>{err}</div>}
        <button onClick={submit} style={{width:"100%",marginTop:12,background:"var(--red)",color:"#fff",border:"none",borderRadius:12,padding:"13px 0",fontWeight:800,fontSize:14,cursor:"pointer"}}>حفظ المنتج</button>
      </div>
    </div>
  );
}
...
function EditProductModal({product,onClose,onSave}){
  ...
  const handleFile=(e)=>{
    ...
    if(!file.type.startsWith("image/")) return setErr("اختر صورة فقط");
    if(file.size>2*1024*1024) return setErr("حجم الصورة كبير (max 2MB)");
    const r=new FileReader(); r.onload=()=>{ setImg(r.result); setPreview(r.result); setErr("") }; r.readAsDataURL(file);
  };
  const submit=()=>{
    if(!name.trim()||!price) return setErr("أدخل الاسم والسعر");
    onSave({...product,name,price:Number(price),quantity:Number(quantity)||0,desc,img});
  };
  const s=fieldStyle();
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(12,14,11,.5)",backdropFilter:"blur(6px)",display:"flex",alignItems:"flex-end",zIndex:50}}>
      <div style={{maxWidth:440,width:"100%",margin:"0 auto",background:"#fff",borderRadius:"20px 20px 0 0",padding:16,border:"1px solid var(--line)",boxShadow:"0 -16px 40px rgba(0,0,0,.18)",maxHeight:"92vh",overflowY:"auto"}}>
        ...
        <label style={{display:"flex",alignItems:"center",gap:10,background:"#F8FAFC",border:"1px dashed var(--line)",borderRadius:12,padding:10,cursor:"pointer",marginBottom:10}}>
          <div style={{width:48,height:48,borderRadius:10,background:`url(${preview}) center/cover`,border:"1px solid var(--line)",flexShrink:0}}/>
          <div style={{flex:1}}>
            <div style={{fontWeight:700,fontSize:12.5,display:"flex",alignItems:"center",gap:6}}><Upload size={13}/> تغيير الصورة</div>
        ...
        {err && <div style={{marginTop:10,background:"var(--red-soft)",border:"1px solid #FECACA",color:"var(--red)",fontSize:12.5,fontWeight:700,borderRadius:10,padding:"9px 11px"}}>{err}</div>}
        <button onClick={submit} style={{width:"100%",marginTop:12,background:"var(--ink)",color:"#fff",border:"none",borderRadius:12,padding:"13px 0",fontWeight:800,fontSize:14,cursor:"pointer"}}>حفظ التعديلات</button>
      </div>
    </div>
  );
}
